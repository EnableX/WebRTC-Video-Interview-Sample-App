...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return EnxDa; });
...
class EnxDa {
  ...
  init() {
    ...
    let server_url = null;
    let socket = null;
    this.clickToCall = function (data) {
      callData = data;
      callData.stall_id = [callData.stall_id];
      if (callData.btn) {
        const buttonSelector = document.getElementById(callData.btn.id);
        buttonSelector.innerHTML = `<button type="button" class="btn btn-success btn-lg">${callData.btn.label}</button>`;
        buttonSelector.addEventListener("click", connectCall);
      }
      server_url = callData.service;
      delete callData.btn;
      connectServer();
    };

    function connectServer() {
      socket = socket_io_client__WEBPACK_IMPORTED_MODULE_0___default.a.connect(server_url, socket_opt);
      socket.on("connected", function (data) {
        console.log("socket connected.....");
        socket.emit("customer-logged-in", callData);
      });
      socket.on("join-call", function (data) {
        console.log(data, "join call data");
        sweetalert2__WEBPACK_IMPORTED_MODULE_1___default.a.fire({
          title: "Agent Available",
          text: "redirecting to new window for join call",
          icon: "success",
          showConfirmButton: false,
          timer: 3000,
        }).then((result) => {
          window.open(callData.service + `/customer/?token=${data.token}`);
        });
      });
    ...
    function connectCall(params) {
      socket.emit("connect-agent", {
        id: callData.user_id,
        stall_id: callData.stall_id,
        name: callData.customer,
      });
    }
  }
}
...
const EnxDa = __webpack_require__(/*! ./components/enxDa */ "./src/components/enxDa.js").default;
module.exports = EnxDa;
